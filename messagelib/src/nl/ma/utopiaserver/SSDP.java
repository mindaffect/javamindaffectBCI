...
public class SSDP implements Runnable {
    ...
    public static List<Map<String,String>> discover(String searchTarget, int timeout, int retries, boolean returnfirst, boolean ipv6) throws IOException {
        ...
        for ( int i=0; i<retries; i++) {
            ...
            if( VERB>1 ) System.out.println(TAG+msearch);
            clientSocket.send(sendPacket);

            /* Receive all responses */
            if (VERB > 0) System.out.println(TAG + "Waiting for responses");
            clientSocket.setSoTimeout(timeout); // limit wait time 
            while (true) {
                try {
                    DatagramPacket receivePacket = new DatagramPacket(receiveData, receiveData.length);
                    clientSocket.receive(receivePacket);
                    if (VERB > 1)
                        System.out.println(TAG + "Got response:" + new String(receivePacket.getData()));
		...
		System.out.println("Exception making server socket: " + e);
            ...
            while (true ) { 
                try {
                    ...
                    if( VERB>0 ) System.out.println(TAG+"Got query packet:\n"+new String(packet.getData()));
    ...
    public void processRequest(String requeststr, SocketAddress sendAdd) throws IOException, SocketException {        
        Map<String,String> request = SSDP.parseMessage(requeststr);
        if ( issearchMatch(request) ) {
            if( VERB>0 )  System.out.println(TAG+"Query matched.  Sending respnose:");
            ...
            DatagramPacket packet = new DatagramPacket(sendData,sendData.length,sendAdd);
    ...
    public void processRequest(DatagramChannel ch) throws IOException, SocketException {
        ...
        processRequest(new String(buffer.array(),"UTF-8"),sendAdd);
    }
    public void processRequest(DatagramPacket packet) throws IOException, SocketException {
        processRequest(new String(packet.getData()),packet.getSocketAddress());
    }
    ...
    public static void main(String[] argv){
        ...
        if ( argv.length>1 ) { // run the server
            ...
            String descriptionxml=server.getXMLDescription("utopia/1.1","MindAffect","http://www.mindaffect.nl","Utopia-HUB BCI server","Utopia-HUB_v1.0","",null);
