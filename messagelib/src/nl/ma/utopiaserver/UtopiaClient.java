...
public class UtopiaClient implements Runnable {
    ...
    public boolean connect(String host, int port) throws IOException {
		...
		sel = Selector.open();
            ...
            tcpChannel.setOption(java.net.StandardSocketOptions.TCP_NODELAY,true);
            tcpChannel.setOption(StandardSocketOptions.SO_RCVBUF,2*MAXBUFFERSIZE); // 8k recieve buffer
   ...
   /**
    ...
    public void sendHeartbeatIfTimeout() throws IOException {
        ...
        if( ts > nextHeartbeatTime ) {
            ...
            sendHeartbeats(null,false); // heartbeats on UDP
        }        
    }
    ...
    public static void main(String argv[]) throws Exception {
        ...
        if( argv.length>1 ) {
            String spawnserver=argv[1];
            if( test.equals("spawnserver") ) {
                test=spawnserver;
                spawnserver="spawnserver";
            }
            System.out.println("Spawnserver:"+spawnserver);
            if( spawnserver.equals("spawnserver") ) {
        ...
        if( test.contains("data") ) {
            System.out.println("Running the fake data streamer");
            float fsample=120;
            int   nchannels=4;
		...
		//System.out.println(s+") nst="+nextSampTime_ms+" curt="+utopiaClient.gettimeStamp()+" sd="+sleepDuration_ms);
			...
			nextSampTime_ms = utopiaClient.gettimeStamp();
                ...
                if( triggerchannel!=null ) {
                    buf.clear();triggerchannel.receive(buf);buf.flip();
                    if(buf.remaining()>0){
            ...
            if( t > nextLogTime_ms ){
					 System.out.print(nSamp + " " + nBlk + " " + ((t-t0)/1000f) + " " + nSamp*1000/((t-t0)+1) + " (samp,blk,sec,hz)\n");
                System.out.flush();
