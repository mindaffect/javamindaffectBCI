...
public class UtopiaServer implements Runnable {
    ...
    final int port;    // default server port
    ...
    final static int MAXBUFFERSIZE=1024*1;
    ...
    public final static int DEFAULTWAITTIMEOUT=1000;
    ...
    final static int[] UTOPIAEXCLUDEFORWARDMESSAGES = { Heartbeat.MSGID, TickTock.MSGID } ;
    ...
    boolean flushInMessageQueue=true;
	...
	String logFileName=DEFAULTLOGFILENAME;
        ...
        UtopiaServer utopiaServer = new UtopiaServer(port,logFileName);
        utopiaServer.run();
    }
    ...
    public UtopiaServer() { this(-1, DEFAULTLOGFILENAME); }
    public UtopiaServer(final int port) { this(port, DEFAULTLOGFILENAME); }
    public UtopiaServer(final int port, String logFileName) {
        if( port > 0 ) this.port = port;  else this.port = DEFAULTPORT;
        msgbuffer = ByteBuffer.allocate(MAXBUFFERSIZE);  msgbuffer.clear();
        tmpbuffer = ByteBuffer.allocate(MAXBUFFERSIZE);  tmpbuffer.clear();
        inmessageQueue = new LinkedList<ServerUtopiaMessage>();
        tsclock = new TimeStampClock();
	...
	// setup the TCP listening socket
        try {
            ...
            showDialog("Utopia Server Listening on:","Address : \n"+ hostIDs);
            System.out.println(TAG+" Utopia Server listening on: \n" + hostIDs);
        } catch (IOException e) {
        ...
        try {
           ...
           NetworkInterface ni=SSDP.getMultiCastNICs();
            System.out.println("SSDP NIC: "+ni);
            // setup the socket we're listening for connections on.
            ssdpChannel = DatagramChannel.open(java.net.StandardProtocolFamily.INET);
            ssdpChannel.setOption(java.net.StandardSocketOptions.SO_REUSEADDR,true);
            ssdpChannel.bind(new InetSocketAddress(SSDP.SSDP_MULTICAST_PORT));
            ssdpChannel.setOption(java.net.StandardSocketOptions.IP_MULTICAST_IF,ni);
        ...
        try {
            ...
            NetworkInterface ni = mcs.getNetworkInterface();
            // setup the socket we're listening for connections on.
            ssdpChannelv6 = DatagramChannel.open(java.net.StandardProtocolFamily.INET6);
            ssdpChannelv6.setOption(java.net.StandardSocketOptions.SO_REUSEADDR,true);
            ssdpChannelv6.bind(new InetSocketAddress(SSDP.SSDP_MULTICAST_PORT));
            ssdpChannelv6.setOption(java.net.StandardSocketOptions.IP_MULTICAST_IF,ni);
        ...
        if (ssdpChannel!=null || ssdpChannelv6!=null ) {
            ...
            try {
                ...
                java.nio.file.Files.write(java.nio.file.Paths.get(descriptionxmlfname),descriptionxml.getBytes("UTF-8"));
     ...
     */
    public int getNewMessages(){ return getNewMessages(0); }
    /**
        ...
        for ( SelectionKey key : this.sel.selectedKeys() ) {
            if (key.isAcceptable()) {
                ...
                try{
                    ...
                    if ( VERBOSITY>2 ) showDialog("New Client Connection", "Client Address: " + clientID);
                    ...
                    ch.setOption(StandardSocketOptions.SO_RCVBUF,MAXBUFFERSIZE*8); // 64k recieve buffer
                    ch.setOption(StandardSocketOptions.SO_SNDBUF,MAXBUFFERSIZE*8); // 64k send buffer
                ...
                if( key.channel() instanceof SocketChannel ) { // only write queue for sockets
                    ...
                    try {
                        synchronized (ci.outmessageQueue ) {
                            if ( VERBOSITY>0 ) System.out.println("Send "+ci.outmessageQueue.size()+" ->"+ci.clientID); 
                ...
                if( key.channel() instanceof DatagramChannel ){
                    if ( key.attachment() instanceof SSDP ) { // SSDP multicast channel
                        ...
                        try { 
                            ssdp.processRequest((DatagramChannel)key.channel());
                        } catch ( IOException ex ) {
    ...
    void processNewMessages(LinkedList<ServerUtopiaMessage> newMessages) {
        ...
        // log *before* rewriting the time-stamps...
    ...
    public void logNewMessage(ServerUtopiaMessage msg){
        if ( VERBOSITY>=0 ) {
            System.out.print((char)msg.clientmsg.msgID());System.out.flush();
        }
        if( logWriter!=null ) {
     ...
     */
    public int gettimeStamp(){ return (int)tsclock.getTime(); }
     ...
     */
    public ServerUtopiaMessage[] popMessages(){
        ServerUtopiaMessage[] messageArray=null;
        synchronized (inmessageQueue) {
            if( !inmessageQueue.isEmpty() ){
                messageArray = inmessageQueue.toArray(new ServerUtopiaMessage[0]);
                inmessageQueue.clear();
            }
        }        
        return messageArray;
     ...
     */
    public boolean messageQueueIsEmpty(){ return this.inmessageQueue.isEmpty(); }
    ...
    public boolean sendMessageOtherClients(UtopiaMessage msg, String exceptClientID){        
        ...
        if( VERBOSITY>= 2 ) System.out.println(TAG+" Sending: " + msg + " to all clients EXCEPT " + exceptClientID) ;
    ...
    public int writeMessageTCP(SocketChannel ch, UtopiaMessage msg) throws IOException {
        int nwrite=-1;
        synchronized ( ch ) {
            if( VERBOSITY>1 ) System.out.println(TAG+" TCP: Sending " + msg.toString() + " -> " + ch.socket().getRemoteSocketAddress());
    ...
    public void writeMessageUDP(Socket clientSocket, UtopiaMessage msg) throws IOException {
        if( VERBOSITY>1 ) System.out.println(TAG+" UDP: Sending " + msg.toString() + " -> " + clientSocket.getRemoteSocketAddress());
    ...
    void showDialog(String title, String msg) {
        try {
            ...
            // TODO: [] make this robust to X11 server not being available?
