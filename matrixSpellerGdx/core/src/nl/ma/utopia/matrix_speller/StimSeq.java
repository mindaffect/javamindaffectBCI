...
/* ToDo:
 [X] - eventSeq - include boolean eventSeq which says at which event times we should
 ...
 [X] - add utilities to modify simply modify the timing
*/
...
public class StimSeq {
    ...
    public float[][] stimSeq=null; // i.e. dim=1 is time, dim=2 is state, hence stimSeq[ei] gives stat at this time.
    ...
    public boolean[] eventSeq=null; // [ nEvent x 1 ]
    ...
    public static StimSeq mkStimSeqSSEP(int nSymb, float seqDuration, float isi, float[] period, float[] phase, boolean smooth){
        ...
        for ( int ei=0; ei<nEvent; ei++){
            ...
            for ( int si=0; si<nSymb; si++) {
                ...
                if( period[si]>0 ) { // period=0 -> never on
    ...
    // Helper function, to make all symbs flicker at the same rate
    public static StimSeq mkStimSeqSSEP(int nSymb, float seqDuration, float isi, float period){
        float[] periods = new float[nSymb];
        for ( int i=0; i<periods.length; i++ ) periods[i]=period;
        return mkStimSeqSSEP(nSymb,seqDuration,isi,periods,null,false);
        
    }
    // Helper function to make only the target flicker at the given rate
    public static StimSeq mkStimSeqSSEP(int nSymb, float seqDuration, float isi, int tgt, float period){
        float[] periods = new float[nSymb];
        periods[tgt] = period; 
        return mkStimSeqSSEP(nSymb,seqDuration,isi,periods,null,false);
    }
    ...
    public StimSeq phaseShiftKey(boolean speedup){
        ...
        for ( int ei=0; ei<stimTime_ms.length; ei++){
            ...
            // update the stimSeq
            float[] ei2   = new float[stimSeq[ei].length];
            float[] ei2p1 = new float[stimSeq[ei].length];
            // loop over stimuli
            for ( int si=0; si<stimSeq[ei].length; si++){
                if ( stimSeq[ei][si]>0 ){ // maps 1 to high-low
                    ei2  [si] = stimSeq[ei][si];
                    ei2p1[si] = 0;
                } else {  // map 0 -> low-high
                    ei2  [si] = 0;
                    ei2p1[si] = 1;
    ...
    public void setIsi_ms(float isi_ms){
        // re-set the isi to the given value.
        for ( int i=0; i<stimTime_ms.length; i++){
            stimTime_ms[i] = (int)(i*isi_ms);
        }
    }
    ...
    public static float[][] readArray(BufferedReader bufferedReader) throws IOException {
        ...
        while ( (line = bufferedReader.readLine()) != null ) {
            ...
            String[] values = line.trim().split("[ ,	]"); // split on , or white-space
